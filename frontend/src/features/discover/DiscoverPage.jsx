...
function _isDebouncedActive(f) {
  return Boolean(
    f.search ||
    f.genres?.length > 0 ||
    f.year_gte || f.year_lte ||
    f.rating_gte ||
    f.with_person ||
    (f.sort_by && f.sort_by !== 'popularity.desc'),
  );
}

/**
 * Returns up to `count` games similar to `selectedId`, scored by
 * overlapping theme + mood tags.
 */
...
/** Game suggestion chip for the "Similar games" row */
...
export default function DiscoverPage() {
  ...
  // ── Selected game (lifted for cross-section control) ──────────────────────
  ...
  // Manual debounce via useRef to avoid importing useDebounce from a separate file
  const [debouncedFilters, setDebouncedFilters] = useState(filters);
  const debounceTimer = useRef(null);
  useEffect(() => {
    clearTimeout(debounceTimer.current);
    debounceTimer.current = setTimeout(() => setDebouncedFilters(filters), 450);
  ...
  // ── Load personalised recs when user logs in ───────────────────────────────
  useEffect(() => {
    if (!isAuthenticated) {
      setPersonalRecs([]);
      return;
  ...
  // ── Fetch grid (always, content driven by filters) ────────────────────────
  useEffect(() => {
    let cancelled = false;
    setGridLoading(true);
  ...
  return (
    <div className="min-h-screen bg-canvas">
      ...
      <div className="max-w-screen-xl mx-auto px-5 sm:px-8 lg:px-12 py-10 space-y-14">
        ...
        {/* ── 1. Game context: Tonight after playing... ───────────────────── */}
        <GameSelector
          selectedId={selectedGameId}
          onGameChange={handleGameChange}
        />

        {/* ── 2. Similar games suggestion row ─────────────────────────────── */}
        <section>
          <div className="mb-4">
        ...
        {/* ── 4. Recommended for you (auth-gated) ─────────────────────────── */}
        ...
        {/* ── 5. Active filter pills (shown above grid when filters active) ── */}
        {hasActiveFilters && (
        ...
        {/* ── 6. Main grid (always visible, infinite scroll) ──────────────── */}
        <section>
          {/* Grid header */}
          <div className="mb-6">
